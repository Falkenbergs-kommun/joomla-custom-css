...
- **Body-text lists vs. menus** (externwebb): editor lists are class-less `<ul>`/`<ol>`, while every UIkit navigation list carries a class (`uk-nav`, `uk-navbar-nav`, `uk-subnav`, `uk-breadcrumb`, `uk-list`, `uk-dotnav`). Content migrated from SiteVision (`div.sv-text-portlet-content`) carries inert classes on lists (`normal`, `bulletList`, Word's `BulletListStyle*`) that must be treated as body text, while `contactInfoList__*` and `blueIconBox__items` also carry `normal` but are styled components. Scope body-text list rules as `#tm-main :is(ul, ol):is(:not([class]), .normal, .bulletList, [class*="BulletListStyle"]):not(nav *):not(#cludo-search-results *):not(.contactInfoList__items):not(.contactInfoList__pages):not(.blueIconBox__items) > li + li` — `:not(nav *)` covers the class-less nested `ul` in the JP Automatic TOC and Cludo's facet list, and `li + li` keeps the space above the first and below the last item unchanged. The same scoping (with `li a:not(.uk-button)`) gives list links the paragraph-link look, since the customizer only styles `p a`. Verify with a headless-Chrome match count on a content page, the start page and `/search#?cludoquery=…` (see the "LISTOR I BRÖDTEXT" block in `externwebb/custom.css`).
